...
Zep node/edge list APIs use UUID cursors. This module follows pages (with per-page retries)
until the full list is loaded, so callers see a single flat list.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch one page; retry transient errors with exponential backoff."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Page through graph nodes; return at most max_items (default 2000). Each page is retried."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Page through all edges for a graph. Each page is retried."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
