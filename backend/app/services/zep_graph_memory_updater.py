"""
Stream simulation agent actions into a Zep graph as text episodes.
"""
...
@dataclass
class AgentActivity:
    """One agent action parsed from the simulation log."""
    ...
    def to_episode_text(self) -> str:
        """
        Natural-language line for Zep ingestion.

        No synthetic "simulation" prefix so graph extraction stays faithful.
        """
        action_descriptions = {
...
class ZepGraphMemoryUpdater:
    """
    Batches AgentActivity rows and calls Zep graph.add with newline-joined text.
    """
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        logger.info(f"ZepGraphMemoryUpdater ready: graph_id={graph_id}, batch_size={self.BATCH_SIZE}")
    ...
    def add_activity(self, activity: AgentActivity):
        """
        Enqueue a meaningful social action.

        Skips DO_NOTHING. action_args should include post/comment text when available.
        """
    ...
    def _worker_loop(self):
        while self._running or not self._activity_queue.empty():
            try:
                ...
                logger.error(f"Zep memory worker loop error: {e}")
                time.sleep(1)
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Zep batch upload failed (attempt {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"Zep batch upload failed after {self.MAX_RETRIES} attempts: {e}")
...
class ZepGraphMemoryManager:
    """Process-wide registry of ZepGraphMemoryUpdater instances keyed by simulation_id."""
