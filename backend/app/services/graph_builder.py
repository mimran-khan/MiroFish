"""
Graph build service.
API #2: Build Standalone Graph using the Zep API.
"""
...
class GraphBuilderService:
    ...
    def _build_graph_worker(
        ...
        try:
            ...
            # 2.5 Detect and convert tabular data
            if TextProcessor.is_tabular(text):
                self.task_manager.update_task(
                    task_id,
                    progress=17,
                    message="Detected tabular data, converting to narrative text..."
                )
                text = TextProcessor.convert_tabular_to_narrative(text)
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # Required by the Zep SDK; warnings come from dynamic class creation and can be ignored.
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            # Build attribute dict and type annotations (Pydantic v2)
    ...
    def add_text_batches(
        ...
        """Add text to the graph in batches; returns all episode UUIDs."""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        while pending_episodes:
            ...
            # Check each episode's processing state
            for ep_uuid in list(pending_episodes):
                try:
                    ...
                    # Ignore per-episode query errors and continue
                    pass
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Fetch full graph data (including detail).

        Args:
            graph_id: Graph ID.

        Returns:
            Dict with nodes and edges, including timestamps, attributes, and other detail.
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # Build node map for resolving node names
        node_map = {}
        for node in nodes:
