...
Uses project context; server-side persisted state.
"""
    ...
    try:
        ...
        def build_task():
            ...
            try:
                ...
                # Detect tabular data and convert to narrative
                build_text = text
                if TextProcessor.is_tabular(build_text):
                    build_logger.info(f"[{task_id}] Detected tabular data, converting to narrative...")
                    task_manager.update_task(
                        task_id,
                        message="Detected tabular data, converting to narrative text...",
                        progress=3
                    )
                    build_text = TextProcessor.convert_tabular_to_narrative(build_text)
                    build_logger.info(f"[{task_id}] Narrative conversion complete: {len(build_text)} chars")
                ...
                # Wait for Zep (per-episode processed flag)
                task_manager.update_task(
                    task_id,
