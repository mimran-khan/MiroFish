...
// Read maxRounds from query on init so child gets value immediately
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog('Failed to close environment gracefully; forcing stop...')
        try {
...
const handleNextStep = () => {
  // Step3Simulation handles report generation and navigation; fallback log only
  ...
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        addLog(`Time config fetch failed; using default: ${minutesPerRound.value} min/round`)
      }
...
const loadGraph = async (graphId) => {
  // While simulating, skip full-screen loading on auto-refresh to avoid flicker
  if (!isSimulating.value) {
...
const startGraphRefresh = () => {
  ...
  // Poll every 30s (first tick after interval)
