    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualization (shown when data exists) -->
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">The graph will build automatically when generation completes</p>
          </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After documents are uploaded, the LLM analyzes their content and automatically generates an ontology suited to public-opinion simulation (entity types + relation types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API description</div>
                <div class="detail-content">
                  Using the generated ontology, documents are chunked and the Zep API builds the knowledge graph to extract entities and relationships.
                </div>
              </div>
...
// Current project ID (may change from 'new' to the real ID)
...
// Initialize — new project or load existing
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // New project: pending upload data from store
...
// New project — call ontology/generate API
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No files pending upload. Return to the home page and try again.'
    loading.value = false
    return
  }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Resume polling for in-progress build
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Poll graph data (independent of task status polling)
      startGraphPolling()
...
const startGraphPolling = () => {
  ...
  // Poll graph data every 10 seconds
...
const fetchGraphData = async () => {
  try {
    // Load project first to get graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Show completion while loading graph
        buildProgress.value = {
          progress: 100,
...
const renderGraph = () => {
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // Preserve raw node
  }))
  
  // Valid node IDs for edge filtering
  ...
  // Edges (visible line + wide transparent hit target)
  ...
  // Wide transparent line for clicks
  ...
  simulation.on('tick', () => {
    // Update edge geometry (visible + hit lines)
