<template>
  ...
  >
    <!-- Tech grid background (when projects or loading) -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Card header: id + feature availability -->
        ...
        <!-- Title from first 20 chars of requirement -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- Description: requirement preview -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 (Start Simulation) and Step 5 (Deep Interaction) must be started during a run; they are not available from history.</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // Selected row for modal
let observer = null
let isAnimating = false  // Animation guard
let expandDebounceTimer = null  // Debounce timer
let pendingState = null  // Pending expand/collapse target
...
// Container min-height from layout mode
const containerStyle = computed(() => {
  ...
  // Expanded: height from row count
  ...
  // Rows × card height + gaps + padding
  const expandedHeight = rows * CARD_HEIGHT + (rows - 1) * CARD_GAP + 10
  
  return { minHeight: `${expandedHeight}px` }
})

// Per-card transform for stack vs grid
const getCardStyle = (index) => {
  const total = projects.value.length
  
  if (isExpanded.value) {
    // Expanded: grid positions
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'

    const col = index % CARDS_PER_ROW
    const row = Math.floor(index / CARDS_PER_ROW)
    
    // Cards in row for centering
...
// Progress badge class from rounds
...
// IntersectionObserver for expand
const initObserver = () => {
  if (observer) {
    observer.disconnect()
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Record latest target expand state
        pendingState = shouldExpand
        
        // Reset debounce on new scroll intent
        ...
        // Debounce expand toggle
        // Shorter delay expand, longer collapse
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Unlock and drain pending
          setTimeout(() => {
            isAnimating = false
            
            // Re-apply pending after animation
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay before chained toggle
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // Multi-threshold intersection
      threshold: [0.4, 0.6, 0.8],
      // rootMargin delays expand
...
/* Square grid via linear-gradient */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
...
.status-icon:nth-child(2).available { color: #F59E0B; } /* Env setup */
.status-icon:nth-child(3).available { color: #10B981; } /* Report */
