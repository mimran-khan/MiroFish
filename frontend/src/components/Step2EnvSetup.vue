    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using context, tools pull entities and relations from the knowledge graph, initialize agents, and ground them in real-world seeds with distinct behavior and memory.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM sets world time flow, recommendation algorithms, per-agent active windows, posting rates, and event triggers from requirements and seeds.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">Agents active / hour</span>
            ...
            <div v-if="simulationConfig.generation_reasoning" class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">LLM config rationale</span>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            From the narrative direction, generate seed events and hot topics to bootstrap the world.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Round count (shown after config and computed rounds exist) -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation rounds</span>
                <span class="section-desc">MiroFish plans <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> h of simulated time; each round is <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> min of elapsed time.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>At ~100 agents: about {{ Math.round(customMaxRounds * 0.6) }} min estimated</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        At ~100 agents: about {{ Math.round(autoGeneratedRounds * 0.6) }} min estimated
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">First run: switch to custom mode to use fewer rounds for a quick preview and lower failure risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              ...
              <span class="info-value">{{ selectedProfile.age || '-' }} y</span>
            </div>
            <div class="info-item">
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                <span class="dim-title">Distinctive memory</span>
...
// State
const phase = ref(0) // 0 init, 1 profiles, 2 config, 3 orchestration, 4 ready
...
// Log deduplication: last emitted progress keys
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulation round count UI
const useCustomRounds = ref(false) // Default: auto round count
const customMaxRounds = ref(40)   // Suggested default: 40 rounds

const PREPARE_STAGE_KEYS = new Set(['reading', 'generating_profiles', 'generating_config', 'copying_scripts'])
/** Backend may send localized stage labels; map to English keys (Unicode escapes, no CJK in source). */
const PREPARE_STAGE_LABEL_TO_KEY = {
  '\u8bfb\u53d6\u56fe\u8c31\u5b9e\u4f53': 'reading',
...
const resolvePrepareStageKey = (fragment) => {
  if (!fragment) return ''
  const t = String(fragment).trim()
  if (PREPARE_STAGE_KEYS.has(t)) return t
  return PREPARE_STAGE_LABEL_TO_KEY[t] || t
}

// Watch API stage key to update phase (avoids matching localized display strings)
watch(prepareStageKey, (key) => {
  if (key === 'generating_profiles') {
    phase.value = 1
  } else if (key === 'generating_config') {
    phase.value = 2
    if (!configTimer) {
      addLog('Starting dual-platform simulation config generation...')
      startConfigPolling()
    }
  } else if (key === 'copying_scripts') {
    phase.value = 2
  }
})

// Auto round count from config (no hardcoded default duration)
...
// Resolve display username by agent_id index
...
// Total interested-topics count across profiles
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Existing preparation found; loading prepared data')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        currentStage.value = detail.current_stage_name || ''
        prepareStageKey.value = detail.current_stage
          ? detail.current_stage
          : resolvePrepareStageKey(detail.current_stage_name || '')
        ...
        if (match) {
          const frag = match[3].trim()
          currentStage.value = frag
          prepareStageKey.value = resolvePrepareStageKey(frag)
        }
    ...
    console.warn('Poll prepare status failed:', err)
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agent profile ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || 'Unknown role'})`)
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          ...
          addLog('Calling LLM for simulation config parameters...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ Simulation duration: ${data.summary.simulation_hours} h`)
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup complete; you can start simulation')
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          addLog(`  ├─ Simulation duration: ${res.data.summary.simulation_hours} h`)
