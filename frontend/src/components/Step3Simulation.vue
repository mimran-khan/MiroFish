          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- Fallback: unknown type or unhandled content -->
...
const props = defineProps({
  ...
  minutesPerRound: {
    type: Number,
    default: 30 // Default minutes per round: 30
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const actionIds = ref(new Set()) // Action IDs for deduplication
...
// Format simulated elapsed time from rounds × minutes per round
...
// Reset all state (restart simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  ...
  // Reset so prior run does not affect this run
  resetAllState()
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Cleared previous simulation logs; restarting')
      }
...
// Track last round per platform for logging
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Log when each platform's round advances
      ...
      // Detect completion via runner_status or platform flags
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // If runner_status lags, infer from platform completed flags
      // Using twitter_completed / reddit_completed
...
// Whether all enabled platforms finished
const checkPlatformsCompleted = (data) => {
  ...
  // If any finished, require all enabled to finish
  // Enabled if actions_count > 0 or was running or completed
  ...
  // All enabled platforms must be complete
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Full list from all_actions
      const serverActions = res.data.all_actions || []
      
      // Append new actions with dedup
...
const handleNextStep = async () => {
  ...
  try {
    ...
    addLog(`✗ Error starting report generation: ${err.message}`)
    isGeneratingReport.value = false
  }
}
