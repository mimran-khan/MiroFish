    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG short- and long-term memory live updating' : 'Live updating...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A few items are still processing; refresh the graph manually in a moment</span>
...
const expandedSelfLoops = ref(new Set()) // Expanded self-loop rows
const showSimulationFinishedHint = ref(false) // Hint after simulation ends
const wasSimulating = ref(false) // Track prior simulating state
...
// Watch isSimulating for simulation end
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transition simulating -> not simulating: show hint
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle self-loop row expand/collapse
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Reset expanded rows
}
...
const renderGraph = () => {
  ...
  // Edge data: pair counts and indices
  const edgePairCount = {}
  ...
  // Index of current edge per node pair
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Nodes with merged self-loop
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // One merged self-loop edge per node
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Full self-loop edge list
        }
      })
      return
    }
    ...
    // Edge direction vs sorted pair (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Curvature: spread multi-edges; straight if single
    let curvature = 0
    if (totalCount > 1) {
      // Even curvature spread for visibility
      // Wider curvature range when more edges
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if edge opposes canonical direction
      // Keeps bundles consistent regardless of direction
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Force simulation: link distance from edge multiplicity
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Distance scales with edges between this pair
      // Base 150px + 50px per extra edge
    ...
    // Weak center gravity for clustering
  ...
  const getLinkPath = (d) => {
    ...
    // Self-loop branch
    if (d.isSelfLoop) {
      // Self-loop: arc out and back
      const loopRadius = 30
      // Start right of node, loop back
    ...
    // Control point from distance and multiplicity
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset scaled by distance
    // More edges -> larger relative offset
  ...
  const getLinkMidpoint = (d) => {
    ...
    // B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2 at t=0.5
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Reset prior edge selection style
  ...
  // Refs for toggling label visibility
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position; distinguish click vs drag
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Restart simulation on real drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Cool down simulation only after drag
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Label positions, horizontal text
